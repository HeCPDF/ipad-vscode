...
- A real iOS cross-compile for `@vscode/sqlite3` / `@parcel/watcher` / `@vscode/spdlog` (their bundled `.node` files are linux-x64 and won't load on iOS as-is; kept in the bundle rather than deleted, since deleting isn't a fix) — until then, storage/watcher features they back will not work at runtime. `.github/workflows/experiment-sqlite3-ios.yml` (standalone, `workflow_dispatch` only, not wired into the main build) tracks a real attempt at this using [prebuild-for-nodejs-mobile](https://github.com/staltz/prebuild-for-nodejs-mobile). Progress so far, each step confirmed by reading actual CI failure output, not guessed:
  ...
  - **Currently blocked on**: with the toolchain issue resolved, the build now genuinely reaches real iOS C++ compilation — and fails there instead, on a real compiler-flag conflict: `error: cannot use 'try'/'throw' with exceptions disabled`. Confirmed from the actual compile command line: `-DNAPI_CPP_EXCEPTIONS` is correctly defined (node-addon-api's source correctly opts into `try`/`throw`), but `-fno-exceptions` is also on the command line, disabling exceptions at the language level regardless. Two things ruled out already, not guessed: `binding.gyp`'s `xcode_settings` block doesn't apply (`gyp ERR! stack Error: make failed` confirms this uses gyp's `make` generator, not the Xcode project generator, which silently ignores `xcode_settings`); and explicitly adding `"cflags_cc": ["-fexceptions"]` to the `vscode-sqlite3` target (patched into the local build copy, confirmed present in the resulting `binding.gyp`) made no difference to the compiler invocation at all — same errors, unchanged. So `-fno-exceptions` isn't simply a lower-priority flag losing an ordering fight at the gyp-source level; it's coming from somewhere gyp's target-level `cflags_cc` doesn't reach (most likely baked into the generated Makefile's compile rule by nodejs-mobile-gyp's own iOS toolchain setup, or a `common.gypi`-equivalent processed with append-only semantics). Next step is inspecting the actual generated `Makefile`/`.mk` rule for this target directly, not further gyp-source-level guessing — three targeted attempts at that level (env vars, target dependency, target cflags_cc) have now all failed to move this specific error.
