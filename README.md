...
3. **UI tests (XCUITest)** — `UITests/OpenFolderFlowUITests.swift` actually taps the real "Open Folder…" button in a booted Simulator. Real automated interaction, not just a static screenshot, and it surfaced a genuine, distinct-from-Network-Extensions Simulator limitation: a freshly-booted CI Simulator has no Apple ID / iCloud Drive / Files providers configured, so `UIDocumentPickerViewController` presents an empty popover — no Cancel button, no browsable content, nothing — even though the system genuinely responded to the tap (confirmed via the standard UIKit `PopoverDismissRegion` accessibility element showing up in the dumped tree). The test's assertion was adjusted to match what's actually achievable in this environment rather than assuming full picker content would render. Still doesn't exercise either extension — this test was specifically chosen because it's the one flow that doesn't need them.
