...
3. **UI tests (XCUITest)** — `UITests/OpenFolderFlowUITests.swift` actually taps the real "Open Folder…" button in a booted Simulator and asserts the system document picker sheet comes up, covering the prompt underneath. Real automated interaction, not just a static screenshot. Still Simulator-only, so still doesn't exercise either extension — this test was specifically chosen because it's the one flow that doesn't need them.
