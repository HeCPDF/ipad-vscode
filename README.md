...
2. **Simulator smoke test** — the Simulator needs no code signing, so CI actually boots one, installs the built app, launches it, and captures a screenshot + log as artifacts. **This has actually run and produced a real screenshot** (see the `simulator-smoke-test` artifact on any successful run): a first-launch iPad showing the "Open a folder to start editing" screen — confirming the app launches, doesn't crash, SwiftUI renders, and the `hasWorkspace` gating logic in `ContentView.swift` behaves correctly on a fresh install. `launchd_sim` logs confirm the process actually spawned (`Successfully spawned iPadVSCode[pid]`). This is real, not "should work." But it is **not** evidence that NodeRuntimeExtension/ExtensionHostRuntime work: Apple's Network Extension framework has long-standing, well-known Simulator support gaps, so the two `NEPacketTunnelProvider` extensions activating is not expected to be exercised by this test, screenshot or no screenshot — the screenshot only shows the pre-workspace-selection screen, which doesn't touch either extension.

3. **UI tests (XCUITest)** — `UITests/OpenFolderFlowUITests.swift` actually taps the real "Open Folder…" button in a booted Simulator, and **passes**. Getting it to pass took two real iterations, both grounded in an actual dumped accessibility tree (printed to the plain-text `xcodebuild test` log) rather than guessing: the first assertion (background prompt disappears) was unreliable because a covered view controller can still report as accessibility-"existing"; the second (a "Cancel" button appears) was wrong because a freshly-booted CI Simulator has no Apple ID / iCloud Drive / Files providers configured, so `UIDocumentPickerViewController` presents an empty popover — no Cancel button, no browsable content, nothing. The tree dump showed the system genuinely did respond to the tap, though: a standard UIKit `PopoverDismissRegion` accessibility element appears, which is what the test now asserts on. Still doesn't exercise either extension — this test was specifically chosen because it's the one flow that doesn't need them.
