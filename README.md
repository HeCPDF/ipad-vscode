...
- A real iOS cross-compile for `@vscode/sqlite3` / `@parcel/watcher` / `@vscode/spdlog` (their bundled `.node` files are linux-x64 and won't load on iOS as-is; kept in the bundle rather than deleted, since deleting isn't a fix) — until then, storage/watcher features they back will not work at runtime. `.github/workflows/experiment-sqlite3-ios.yml` (standalone, `workflow_dispatch` only, not wired into the main build) tracks a real attempt at this using [prebuild-for-nodejs-mobile](https://github.com/staltz/prebuild-for-nodejs-mobile). Progress so far, each step confirmed by reading actual CI failure output, not guessed:
  ...
  - **Currently blocked on**: with the toolchain issue resolved, the build now genuinely reaches real iOS C++ compilation — and fails there instead, on a real compiler-flag conflict: `error: cannot use 'try'/'throw' with exceptions disabled`. `vscode-sqlite3`'s `binding.gyp` explicitly depends on node-addon-api's `node_addon_api_except` target (the exceptions-enabled variant — it wants C++ exceptions on), but something in the iOS cross-compile path (nodejs-mobile's or prebuild-for-nodejs-mobile's own common.gypi-equivalent cflags for iOS, not yet located) is forcing `-fno-exceptions` at a level the per-target dependency doesn't override. Next step is capturing the full `--verbose` compiler invocation to see the actual flag ordering (whichever of `-fexceptions`/`-fno-exceptions` comes last on the command line wins), not guessing further.
