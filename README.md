...
- A real iOS cross-compile for `@vscode/sqlite3` / `@parcel/watcher` / `@vscode/spdlog` (their bundled `.node` files are linux-x64 and won't load on iOS as-is; kept in the bundle rather than deleted, since deleting isn't a fix) — until then, storage/watcher features they back will not work at runtime. `.github/workflows/experiment-sqlite3-ios.yml` (standalone, `workflow_dispatch` only, not wired into the main build) tracks a real attempt at this using [prebuild-for-nodejs-mobile](https://github.com/staltz/prebuild-for-nodejs-mobile). Progress so far, each step confirmed by reading actual CI failure output, not guessed:
  ...
  - **Currently blocked on**: the `nodejs-mobile-gyp@0.3.1` this tool version bundles has Python-2-only syntax (`print "..."`) hardcoded into its own version-detection code, so it crashes under any Python 3 interpreter regardless of `PYTHON`/`npm_config_python` env vars — this isn't a config problem, the bundled tool version itself needs to be newer. `prebuild-for-nodejs-mobile`'s own README notes upstream `nodejs-mobile-gyp` was later "updated... to align with node-gyp version 10.0.1 which supports Python 3" — next step is finding/forcing a `prebuild-for-nodejs-mobile`/`nodejs-mobile-gyp` version combination newer than what `18.17.7` pins.
