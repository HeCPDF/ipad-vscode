...
**Current state**: the Swift-side plumbing (ExtensionHostRuntime target, `ExtensionHostTunnelProvider`, `TunnelController`'s Darwin-notification observer, `ExtensionHostLaunchRequest`) compiles and is wired end to end, but **nothing calls it yet** — the actual patch to `extensionHostConnection.ts` (replace the `cp.fork` line with "write the launch request + post the Darwin notification") has not been written. That TS patch is the next concrete step.

**Known-fragile point, not yet resolved**: Darwin notifications aren't guaranteed to wake a suspended app. If the user has backgrounded the app when the exthost needs (re)launching, this hand-off may simply not fire. No workaround identified yet.
